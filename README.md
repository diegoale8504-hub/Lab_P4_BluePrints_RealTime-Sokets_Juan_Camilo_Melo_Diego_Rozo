...
## 🚀 Solución Unificada del Equipo (Juan Camilo Melo & Diego Rozo)

Se ha unificado la funcionalidad completa del **Lab 3 (React UI & CRUD REST)** con el **Lab 4 (Real-Time WebSockets & STOMP/Socket.IO)**.

### 📌 Características Implementadas
1. **Integración REST CRUD Completa**:
   - `GET /api/blueprints?author=:author`: Consulta los planos de un autor.
   - `GET /api/blueprints/:author/:name`: Obtiene los puntos del plano seleccionado.
   - `POST /api/blueprints`: Creación de un nuevo plano vía modal interactivo (`CreateBlueprintModal`).
   - `PUT /api/blueprints/:author/:name`: Guarda/Actualiza los puntos trazados en el Canvas.
   - `DELETE /api/blueprints/:author/:name`: Elimina un plano específico.
   - **Total Puntos del Autor**: Calculado dinámicamente mediante la función `reduce` sobre los planos retornados.

2. **Sincronización en Tiempo Real (RT)**:
   - **STOMP (Spring Boot)**: Suscripción a tópicos `/topic/blueprints.{author}.{name}` y publicación en `/app/draw`.
   - **Socket.IO (Node.js)**: Unión a salas `blueprints.{author}.{name}` (evento `join-room`) y emisión de eventos `draw-event` / recepción de `blueprint-update`.
   - **Selector Dinámico**: Permite cambiar entre STOMP, Socket.IO o deshabilitar RT (solo REST).
   - **Indicador de Estado**: Visualiza en tiempo real si el cliente está `Conectado (RT)`, `Conectando...`, `Desconectado` o `Sin RT Activo`.

3. **Interfaz de Usuario (UI/UX Premium)**:
   - Tema oscuro moderno con tarjetas translúcidas, badges de estado animados y tipografía limpia (`Inter`).
   - Canvas HTML5 con cuadrícula de blueprint, trazado glow de líneas y nodos con coordenadas.
